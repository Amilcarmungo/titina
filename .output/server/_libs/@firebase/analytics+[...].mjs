...
function initializeApp(_options, rawConfig = {}) {
	...
	if (existingApp) {
		if (!deepEqual(options, existingApp.options)) throw ERROR_FACTORY$2.create("duplicate-app", {
			appName: name,
			mismatchedParam: "options",
			oldValue: JSON.stringify(existingApp.options),
			newValue: JSON.stringify(options)
		});
		else if (!deepEqual(config, existingApp.config)) throw ERROR_FACTORY$2.create("duplicate-app", {
			appName: name,

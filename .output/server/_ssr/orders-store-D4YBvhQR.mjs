...
//#endregion
//#region node_modules/.nitro/vite/services/ssr/assets/orders-store-D4YBvhQR.js
var STAGE_FLOW = [
	"awaiting_payment",
	"payment_review",
	"payment_rejected",
...
function notifyCustomer(order, pkg, note) {
	...
	if (order.uid) import("./notifications-dAP2pN34.mjs").then(({ pushNotificationTo }) => pushNotificationTo(order.uid, {
		kind,
		title,
		body,
		href: "/orders"
...
var orderActions = {
	...
	update(id, patch) {
		...
		put(next);
		if (!order.paymentProof && next.paymentProof) packagesOf(next).forEach((pkg) => {

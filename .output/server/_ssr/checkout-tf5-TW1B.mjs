...
import { Bt as ChevronRight, G as Minus, Ht as ChevronDown, T as ShieldCheck, Ut as Check, Y as MapPin, jt as CreditCard, z as Plus } from "../_libs/lucide-react.mjs";
import { v as Link, y as useNavigate } from "../_libs/@tanstack/react-router+[...].mjs";
import { c as actions, u as useStore } from "./router-HhRHfmJx.mjs";
import { t as Layout } from "./Layout-BezCClCD.mjs";
import { i as quoteShipping, l as useShippingSettings, o as shippingOptions } from "./logistics-store-DuC-stwY.mjs";
import { clearCheckoutDraft, listAddresses, saveAddress, saveCheckoutDraft } from "./user-data-DXEcUX7Y.mjs";
import { t as sendAppEmail } from "./send-Dc_Kd1Ui.mjs";
import { orderActions } from "./orders-store-DPNmKvMS.mjs";
import { r as usePaymentMethods } from "./payments-store-BCpcu4oE.mjs";
import { i as setPendingPayment, r as newOrderCode } from "./pending-payment-DaWfZjfD.mjs";
import { r as validateCoupon } from "./coupons-store-K_R4Qx57.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/checkout-tf5-TW1B.js
...
function normalizeSavedAddress(address) {
	...
	const countryCode = address.countryCode && address.countryCode.trim() ? address.countryCode.trim() : rawPhone.startsWith("+") ? rawPhone.split(" ")[0] || "+244" : "+244";
	const phone = rawPhone.startsWith("+") ? rawPhone.slice(countryCode.length).trim() : rawPhone;
	return {
		name: address.name ?? "",
		phone,
		countryCode,
...
function hasCompletedAddress(address) {
	return !!(address.name?.trim() && address.phone?.trim() && address.street?.trim() && address.city?.trim() && address.cep?.trim());
}
...
function CheckoutPage() {
	...
	const [savedAddressConfirmed, setSavedAddressConfirmed] = (0, import_react.useState)(false);
	...
	const goToStep = (0, import_react.useCallback)((next) => {
		if (typeof window !== "undefined" && next < stepRef.current) {
			window.history.back();
			return;
		}
		setStep(next);
		if (typeof window !== "undefined") window.history.pushState({ checkoutStep: next }, "");
	}, []);
	const goToPreviousStep = (0, import_react.useCallback)(() => {
		if (step > 1) {
			setStep(step - 1);
			if (typeof window !== "undefined") window.history.pushState({ checkoutStep: step - 1 }, "");
		}
	}, [step]);
	(0, import_react.useEffect)(() => {
		const onPop = () => {
								...
								children: [
									...
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										...
										children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
											...
											/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
												className: "text-sm text-muted-foreground",
												children: buildAddressSummary(address)
											})
										] }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
											onClick: () => goToStep(1),
					...
					children: [step === 3 ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
						...
						onClick: () => {
							if (step === 1) goToStep(2);
								...
								if (hasCompletedAddress(address)) goToStep(3);
						...
						children: step === 1 ? "Salvar endereço" : step === 2 ? hasCompletedAddress(address) ? "Continuar para o pagamento" : "Preencha o endereço" : "Pagar"
					})]
				})
			})
		]

...
import { n as notificationActions } from "./notifications-store-B_Op6deg.mjs";
import { t as getShop } from "./shops-store-Dm5LimXI.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/orders-store-DPNmKvMS.js
...
function mergeRemoteOrders(remote) {
	const restored = (staffMode ? remote : remote.filter((o) => o.uid === ownerUid)).map((order) => ({
		...order,
...
function notifyCustomer(order, pkg, note) {
	...
	const body = note?.trim() ? note.trim() : `${STAGE_DESC[pkg.stage]}${pkg.eta ? ` ${pkg.eta}.` : ""}`;
	const title = `Pedido #${order.id} — ${STAGE_LABEL[pkg.stage]}`;
	const kind = pkg.stage === "shipped" || pkg.stage === "delivered" ? "delivery" : "order";
	if (order.uid) import("./notifications-Df9V44Mn.mjs").then(({ pushNotificationTo }) => pushNotificationTo(order.uid, {
		kind,
		title,
		body,
		href: "/orders"
	...
	if (to && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(to)) import("./send-Dc_Kd1Ui.mjs").then((n) => n.n).then(({ sendAppEmail }) => sendAppEmail("notification", to, {
		title,
...
function notifyOrderStatus(order, title, body) {
	if (order.uid) import("./notifications-Df9V44Mn.mjs").then(({ pushNotificationTo }) => pushNotificationTo(order.uid, {
		kind: "order",
		title,
		body,
...
var orderActions = {
	add(o) {
		...
		if (firstPackage) notifyCustomer(order, firstPackage);
		return id;
	},
	updateStatus(id, status) {
		...
		if (status === "returns") {
			...
			put(next);
			notifyOrderStatus(next, `Pedido #${next.id} — devolução em análise`, "A sua devolução foi encaminhada para análise. Enviaremos uma nova atualização assim que houver uma decisão.");
			return;
		}
		...
		packages.forEach((pkg, index) => {
			if (pkg.stage !== previous[index]?.stage) notifyCustomer(next, pkg);
		});
	},
	/** Reabre um pedido recusado para o cliente enviar um novo comprovativo. */
	reopenPayment(id, methodId) {
		const order = orders.find((o) => o.id === id);
		if (!order) return;
		const packages = packagesOf(order).map((pkg) => ({
			...pkg,
	...
	markReceived(orderId, packageId) {
		...
		if (!order) return;
		const previous = packagesOf(order);
		const packages = previous.map((p) => (!packageId || p.id === packageId) && p.stage !== "delivered" && p.stage !== "reviewed" ? {
			...p,
			stage: "delivered",
			timeline: [...p.timeline, {
				stage: "delivered",
	...
	update(id, patch) {
		...
		put(next);
		if (!order.paymentProof && next.paymentProof) (next.packages ?? []).forEach((pkg) => {
	...
	requestRefund(id, data) {
		...
		notifyOrderStatus(order, `Pedido #${order.id} — reembolso solicitado`, `Recebemos o seu pedido de reembolso por ${data.method}. A equipa irá analisar os dados e avisará quando houver uma decisão.`);
	},
	remove(id) {
...
//#endregion
export { STAGE_DESC, STAGE_FLOW, STAGE_LABEL, buildPackages, mergeRemoteOrders, orderActions, packagesOf, registerOrdersBridge, setOrdersOwner, statusFromPackages, useOrders };

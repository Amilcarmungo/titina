...
import { n as createServerFn, r as TSS_SERVER_FUNCTION } from "./server-CdD4OYKw.mjs";
import processModule from "node:process";
//#region node_modules/.nitro/vite/services/ssr/assets/support-chat.functions-BIGJVWXL.js
...
var SYSTEM = `És a Jilda IA, a assistente virtual da Bazarixy — um marketplace angolano de moda, beleza, casa e electrónica.
...
var askSupport = createServerFn({ method: "POST" }).validator((data) => schema.parse(data)).handler(askSupport_createServerFn_handler, async ({ data }) => {
	...
	try {
		...
		return { reply: (await res.json()).candidates?.[0]?.content?.parts?.[0]?.text?.trim() || fallbackSupportReply(lastMessage) };
	} catch {

...
import { n as createServerFn, r as TSS_SERVER_FUNCTION } from "./server-DLhr3NiF.mjs";
import processModule from "node:process";
//#region node_modules/.nitro/vite/services/ssr/assets/support-chat.functions-BUPCO9k6.js
...
var askSupport = createServerFn({ method: "POST" }).validator((data) => schema.parse(data)).handler(askSupport_createServerFn_handler, async ({ data }) => {
	...
	try {
		...
		if (!json.candidates?.[0]?.content?.parts?.[0]?.text?.trim()) return { reply: fallbackSupportReply(lastMessage) };
		return { reply: json.candidates[0].content?.parts?.[0]?.text?.trim() ?? context };
	} catch {

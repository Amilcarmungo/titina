...
//#endregion
//#region node_modules/.nitro/vite/services/ssr/assets/orders-store-D4TWKxP-.js
var STAGE_FLOW = [
	"awaiting_payment",
	"payment_review",
	"payment_accepted",
...
function notifyCustomer(order, pkg, note) {
	...
	if (order.uid) import("./notifications-BEc2dD8v.mjs").then(({ pushNotificationTo }) => pushNotificationTo(order.uid, {
		kind,
		title,
		body,
		href: "/orders"
	...
	if (to && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(to)) import("./send-6FcbH887.mjs").then((n) => n.n).then(({ sendAppEmail }) => sendAppEmail("notification", to, {
		title,

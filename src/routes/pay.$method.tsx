  ...
  head: () => ({
    meta: [
      { title: "Pagamento — Bazarixy" },
      {
        name: "description",
        content:
          "Conclua o pagamento do seu pedido e envie o comprovativo para validação.",
      },
...
function PayMethodPage() {
  ...
  const methodId =
    methodParam === "express" ? "multicaixa-express" : methodParam;
  ...
  const finish = async () => {
    if (!pending || !proof || !rulesAccepted || sending || finishLock.current)
      return;
    ...
    const url = await uploadProofFile(
      proof.file,
      storagePaths.proof(uid, pending.code, proof.file.name),
      setProgress,
    );
    setSending(false);
    if (!url) {
      ...
      <Layout hideHeader hideBottomNav>
        <div className="mx-auto max-w-md px-6 py-16 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            O seu pagamento do pedido <b>{done}</b> está a ser avaliado. Assim
            que o comprovativo for validado, a encomenda segue para envio — pode
            acompanhar o estado em «Meus pedidos».
          </p>
          <div className="mt-6 flex gap-3">
            <Link
              to="/orders"
              search={{ tab: "processing" as const }}
    ...
    return (
      <Layout hideHeader hideBottomNav>
        <div className="mx-auto max-w-md px-6 py-20 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Finalize um pedido no checkout para pagar.
          </p>
          <button
    ...
    <Layout hideHeader hideBottomNav>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl bg-card p-4 shadow-[var(--shadow-card)]">
              <h2 className="flex items-center gap-2 text-sm font-black">
                <Info className="h-4 w-4 text-sale" /> Como pagar
              </h2>
              <ol className="mt-2 space-y-1.5 text-xs text-muted-foreground">
                {(method?.instructions
                  ...
                  <li key={i} className="flex gap-2">
                    <span className="grid h-4 w-4 shrink-0 place-items-center rounded-full bg-foreground text-[9px] font-black text-background">
                      {i + 1}
                    </span>
                    <span>{s}</span>
                  </li>
              ...
              <p className="mt-3 flex items-center gap-1.5 text-[11px] text-emerald-600">
                <ShieldCheck className="h-3.5 w-3.5" /> O pedido só é enviado
                após validação do comprovativo.
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-amber-300 bg-amber-50 p-4 text-xs text-amber-950">
              ...
              <ul className="mt-2 list-disc space-y-1 pl-4">
                <li>Transfira exactamente o valor indicado neste pedido.</li>
                <li>
                  Envie apenas um comprovativo real, legível e desta
                  transferência.
                </li>
                <li>
                  É proibido enviar comprovativo falso, editado, repetido ou com
                  valor diferente.
                </li>
              </ul>
          ...
          <div className="space-y-3 rounded-2xl bg-card p-4 shadow-[var(--shadow-card)]">
            ...
            <div className="pt-1">
              ...
              {!proof ? (
                ...
                >
                  ...
                  <span className="text-[11px] text-muted-foreground">
                    Imagem (JPG, PNG) ou PDF · máx. 8 MB
                  </span>
                </button>
              ) : (
              ...
              <p className="mt-2 text-[11px] text-muted-foreground">
                {proof
                  ? "O comprovativo é enviado ao tocar em «Finalizar»."
                  : "Sem comprovativo não é possível finalizar."}
              </p>
            </div>
            ...
            >
              {sending
                ? `A enviar comprovativo… ${progress}%`
                : `Finalizar pedido · ${formatKz(pending.total)}`}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-border bg-background px-3 py-3 md:hidden">
        ...
        >
          {sending
            ? `A enviar… ${progress}%`
            : `Finalizar · ${formatKz(pending.total)}`}
        </button>
      </div>
    </Layout>
  );

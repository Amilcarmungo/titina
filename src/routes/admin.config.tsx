  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-2xl bg-background p-5 shadow-[var(--shadow-card)] space-y-3">
        <label className="block">
          ...
          <input
            value={cfg.storeName}
            onChange={(e) =>
              homeConfigActions.update({ storeName: e.target.value })
            }
        ...
        <label className="block">
          ...
          <input
            value={cfg.currency}
            onChange={(e) =>
              homeConfigActions.update({ currency: e.target.value })
            }
            className="mt-1 w-full rounded-lg border border-border bg-transparent px-3 py-2 text-sm outline-none"
          />
        </label>
      ...
      <section className="rounded-2xl bg-background p-5 shadow-[var(--shadow-card)] space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          O site mostra apenas o que existe no banco de dados. Use estes botões
          para semear exemplos e depois editá-los em Produtos e Categorias.
        </p>
          ...
          <button
            onClick={async () => {
              try {
                ...
                toast.error(
                  (err as Error)?.message ?? "Não foi possível publicar",
                );
              }
            }}
            className="rounded-full border border-border px-4 py-2 text-xs font-bold"

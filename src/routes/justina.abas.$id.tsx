  ...
  head: () => ({
    meta: [
      { title: "Gerir aba da home — Bazarixy Admin" },
      {
        name: "description",
        content:
          "Crie e edite abas principais da página inicial: nome, categorias e banners exclusivos.",
      },
...
function TabEditor() {
  ...
  const tabBanners = useMemo(
    () =>
      slideIds
        .map((sid) => slides.find((s) => s.id === sid))
  ...
  const saveTab = () => {
    ...
    if (isNew) {
      ...
      homeConfigActions.update({
        homeTabs: [
          ...cfg.homeTabs,
          { id: newId, label: label.trim(), slugs, slideIds },
        ],
      });
  ...
  const saveBanner = async () => {
    ...
    const img = bannerFiles.image
      ? await uploadImageFile(
          bannerFiles.image,
          storagePaths.banner(bannerId, bannerFiles.image.name),
          {
            silent: true,
            onProgress: (p) => setPublishProgress(Math.round(10 + p * 0.55)),
          },
        )
      : banner.img;
    const overlay = bannerFiles.overlay
      ? await uploadImageFile(
          bannerFiles.overlay,
          storagePaths.banner(`${bannerId}/overlay`, bannerFiles.overlay.name),
          {
            silent: true,
            onProgress: (p) => setPublishProgress(Math.round(65 + p * 0.2)),
          },
        )
      : banner.overlay;
    if (!img) {
    ...
    if (banner.overlay?.startsWith("blob:"))
      URL.revokeObjectURL(banner.overlay);
    setBannerFiles({});
    ...
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-2xl font-black tracking-tight">
              {isNew ? "Nova aba principal" : label || "Aba"}
            </h1>
            <p className="text-xs text-muted-foreground">
              Nome, categorias e banners exclusivos desta aba.
            </p>
          </div>
        </div>
        <button
      ...
      <section className="rounded-2xl bg-background p-5 shadow-[var(--shadow-card)]">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-wide text-muted-foreground">
              Passo 2 · Categorias desta aba
            </p>
            <p className="mt-0.5 text-[11px] text-muted-foreground">
              Só as categorias escolhidas aparecem. Subcategorias não são
              exibidas nesta aba.
            </p>
          </div>
          <Link
        ...
        <p className="mt-2 text-[11px] text-muted-foreground">
          {slugs.length
            ? `${slugs.length} categoria(s) nesta aba.`
            : "Sem seleção = mostra produtos de todas as categorias."}
        </p>
      </section>
      ...
      <section className="rounded-2xl bg-background p-5 shadow-[var(--shadow-card)]">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-wide text-muted-foreground">
              Passo 3 · Banners desta aba
            </p>
            <p className="mt-0.5 text-[11px] text-muted-foreground">
              Crie aqui os banners que aparecem quando o cliente toca nesta aba.
            </p>
          </div>
          <button
            onClick={() => {
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          ...
          >
            <span className="flex flex-col items-center gap-2">
              <ImageIcon className="h-5 w-5" /> Novo banner
            </span>
          </button>
        </div>
      </section>
          ...
          <div className="grid gap-4">
            {publishing && (
              <div className="rounded-xl border border-brand/40 bg-brand/10 p-3">
                <div className="flex justify-between text-xs font-bold">
                  <span>A publicar banner no Firebase…</span>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <AdminField label="Legenda">
                <AdminInput
                  ...
                  onChange={(e) =>
                    setBanner({ ...banner, caption: e.target.value })
                  }
                />
              </AdminField>
            ...
            <AdminField label="Imagem">
              <div className="flex flex-wrap items-center gap-3">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (f) {
                      ...
                      setBannerFiles((files) => ({ ...files, image: f }));
                      setBanner({ ...banner, img: preview });
                    }
                    e.target.value = "";
                  }}
                />
                ...
                <input
                  ...
                  onChange={(e) =>
                    setBanner({ ...banner, img: e.target.value })
                  }
              ...
              <div className="flex flex-wrap items-center gap-3">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (f) {
                      const preview = URL.createObjectURL(f);
                      setBannerFiles((files) => ({ ...files, overlay: f }));
                      setBanner({ ...banner, overlay: preview });
                    }
                    e.target.value = "";
                  }}
                />
                ...
                {banner.overlay && (
                  <>
                    ...
                    />
                    <button
                      onClick={() => setBanner({ ...banner, overlay: "" })}
            ...
            <AdminField label="Produtos em destaque (até 3)">
              <ProductPicker
                ...
                onChange={(ids) => setBanner({ ...banner, pickIds: ids })}
                max={3}
              />
            </AdminField>
          </div>
      ...
      {publishedLink && (
        <div className="fixed inset-x-4 bottom-5 z-[70] mx-auto max-w-lg rounded-2xl border border-emerald-200 bg-background p-4 shadow-2xl">
          <p className="font-black text-emerald-700">Banner publicado</p>

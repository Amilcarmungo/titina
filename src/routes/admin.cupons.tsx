      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {coupons.length} cupom(ns) · usados no checkout
          </p>
        </div>
        <button
          onClick={() => {
        ...
        {coupons.map((c) => (
          ...
          >
            <div className="flex items-start gap-3">
              <span className="grid h-11 w-11 flex-none place-items-center rounded-xl bg-muted">
                <Ticket className="h-5 w-5" />
              </span>
              <div className="min-w-0 flex-1">
                <p className="truncate font-mono text-sm font-black tracking-wider">
                  {c.code}
                </p>
                <p className="text-[11px] text-muted-foreground">
                  {c.type === "percent"
                    ? `${c.value}% OFF`
                    : `${formatKz(c.value)} OFF`}
            ...
            <div className="mt-3 flex gap-2">
              ...
              <button
                onClick={() =>
                  couponActions.update(c.code, { active: !c.active })
                }
      ...
      >
        {draft && (
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <AdminField
              label={
                draft.type === "percent" ? "Desconto (%)" : "Desconto (Kz)"
              }
            >
              <AdminInput
                type="number"
            ...
            <AdminField label="Pedido mínimo (Kz)">
              <AdminInput
                ...
                onChange={(e) =>
                  setDraft({ ...draft, minOrder: Number(e.target.value) })
                }
              />
            </AdminField>
            <AdminField
            ...
            >
              <AdminInput
                value={draft.expires ?? ""}
                onChange={(e) =>
                  setDraft({ ...draft, expires: e.target.value })
                }
            ...
            <AdminField label="Estado">
              <AdminSelect
                ...
                onChange={(e) =>
                  setDraft({ ...draft, active: e.target.value === "1" })
                }
              >

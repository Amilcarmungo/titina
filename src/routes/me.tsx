...
import {
  Settings, Ticket, Coins, Wallet, Gift,
  CreditCard, Package, Truck, MessageSquare, Undo2,
  Headphones, Repeat2, Store, ShieldCheck, AlertCircle,
  ChevronRight, Heart,
} from "lucide-react";
...
export const Route = createFileRoute("/me")({
  head: () => ({ meta: [{ title: "Minha conta — Bazarixy" }, { name: "description", content: "Sua conta Bazarixy." }] }),
  component: MePage,
});
    ...
    <Layout title="Minha Conta" hideHeader>
        ...
        <div className="flex items-center justify-between gap-2">
          {user ? (
            <div className="flex min-w-0 flex-1 items-center gap-2">
              ...
              <span className="font-display text-base sm:text-lg md:text-xl font-black truncate min-w-0">{displayName}</span>
            </div>
          ) : (
            <Link to="/auth" onClick={onSignInClick} className="flex min-w-0 flex-1 items-center gap-1">
              <span className="font-display text-xl sm:text-2xl md:text-3xl font-black truncate">Sign In / Register</span>
              <ChevronRight className="h-6 w-6 shrink-0" />
            </Link>
          )}
          <Link to="/settings" className="p-2 shrink-0"><Settings className="h-5 w-5" /></Link>
        </div>
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className="rounded-xl bg-white p-3 shadow-[var(--shadow-card)]">
            <div className="flex items-center gap-1.5">
              <span className="grid h-5 w-5 place-items-center rounded-full bg-gold text-[10px] font-black text-white">B</span>
              <span className="font-display text-sm font-black tracking-wider text-gold">BAZARIXY CLUB</span>
            </div>
            ...
            <button className="mt-2 w-full rounded-md bg-orange-50 py-2 text-sm font-bold text-amber-700">Aderir agora</button>
          </div>

          {/* Bazarixy Saver */}
          <div className="rounded-xl bg-white p-3 shadow-[var(--shadow-card)]">
            <div className="flex items-center gap-1">
              <span className="font-display text-sm font-black tracking-wider text-sale">BAZARIXY</span>
              <span className="font-display text-sm font-black italic text-sale">Saver</span>
            </div>
            <p className="mt-2 text-sm">Cupons para você!</p>
            <div className="mt-2 grid grid-cols-2 gap-2">
              {coupons.slice(0, 2).map((c) => (
                <div key={c.code} className="rounded-lg border border-border py-2 text-center">
                  <p className="text-sm font-black text-sale">
                    {c.type === "percent" ? <>{c.value}<span className="text-[10px]">% OFF</span></> : <>Kz {c.value.toLocaleString("pt-AO")}<span className="text-[10px]"> OFF</span></>}
                  </p>
                  <p className="text-[10px] text-muted-foreground">{c.minOrder ? `Pedidos Kz ${c.minOrder.toLocaleString("pt-AO")}+` : "Sem mínimo"}</p>
                </div>
              ))}
              {coupons.length === 0 && <p className="col-span-2 text-[11px] text-muted-foreground">Sem cupons disponíveis agora.</p>}
            </div>
            <Link to="/coupons" className="mt-2 block w-full rounded-md bg-pink-50 py-2 text-center text-sm font-bold text-sale">Ver cupons</Link>
      ...
      <div className="mx-3 md:mx-0 mt-3 rounded-xl bg-white shadow-[var(--shadow-card)] p-3">
        <div className="grid grid-cols-4 gap-2">
          {[
            ...
            <Link to={q.to} key={q.label} className="flex flex-col items-center gap-1 py-2">
      ...
      <div className="mx-3 md:mx-0 mt-3 rounded-xl bg-white shadow-[var(--shadow-card)] p-3">
        <div className="flex items-center justify-between">
          ...
          <Link to="/orders" search={{ tab: "unpaid" as const }} className="flex items-center text-xs text-muted-foreground">Ver todos <ChevronRight className="h-3 w-3" /></Link>
        </div>
        <div className="mt-3 grid grid-cols-5 gap-2">
          {([
            { icon: CreditCard, label: "A pagar", tab: "unpaid" as const },
            { icon: Package, label: "Processando", tab: "processing" as const },
            ...
            return (
              <Link key={o.label} to="/orders" search={{ tab: o.tab }} className="relative flex flex-col items-center gap-1 py-2 text-center">
      ...
      <div className="mx-3 md:mx-0 mt-3 rounded-xl bg-white shadow-[var(--shadow-card)] p-3">
        ...
        <div className="mt-3 grid grid-cols-5 gap-2">
          {[
            ...
            { icon: Repeat2, label: "Trocas", to: "/orders" as const, search: { tab: "returns" as const } },
            { icon: Store, label: "Seguindo", to: "/store" as const },
            { icon: ShieldCheck, label: "Política", to: "/termos" as const },
            { icon: AlertCircle, label: "Avisos", to: "/notifications" as const },
          ].map((s) => (
            <Link
              key={s.label}
              to={s.to}
      ...
      <div className="mx-3 md:mx-0 mt-3 rounded-xl bg-white shadow-[var(--shadow-card)]">
        ...
        <div className="flex flex-col items-center justify-center px-6 py-10 text-center">
          ...
          <p className="mt-3 text-sm text-muted-foreground">Você ainda não salvou nada por aqui.</p>
          <Link to="/" className="mt-4 rounded-full bg-foreground px-6 py-2 text-xs font-bold text-background">Explorar produtos</Link>
        </div>
      </div>
      </div>
    </Layout>

...
import { useAllProducts, useProductsStatus, retryProducts } from "@/lib/products-store";
import { useSlides, setBannerIndex, useBannersStatus, retryBanners } from "@/lib/banner";
import { useHomeConfig, useActiveHomeTab, setActiveHomeTab } from "@/lib/home-config";
import { useCategories, useCategoriesStatus, type CategoryFull } from "@/lib/categories-store";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Compre moda feminina, masculina e infantil com até 70% off. Frete grátis a partir de Kz 120.000." },
      { property: "og:title", content: "Bazarixy — Moda online" },
      { property: "og:description", content: "Tendências da moda com preços imperdíveis." },
    ],
  }),
  component: Home,
});
...
function Home() {
  ...
  const orderedCats = useMemo(() => {
    ...
    const head = cfg.categoriesOrder.map((s) => map.get(s)).filter(Boolean) as CategoryFull[];
  ...
  const tabs = cfg.homeTabs.length ? cfg.homeTabs : [{ id: "t-all", label: "Tudo", slugs: [] as string[] }];
  ...
  const onTouchStart = (e: React.TouchEvent) => { touch.x = e.touches[0].clientX; touch.active = true; };
  ...
  const filtered = useMemo(
    () => (!active.slugs.length ? products : products.filter((p) => active.slugs.includes(p.category))),
    [products, active.slugs],
  );

  const tabCats = useMemo(
    () => (!active.slugs.length ? orderedCats : orderedCats.filter((c) => active.slugs.includes(c.slug))),
    ...
    <Layout transparentHeader>
      ...
      >
        <div className="flex transition-transform duration-700 ease-out" style={{ transform: `translateX(-${i * 100}%)` }}>
          {slides.map((s, idx) => (
            <SmartImage key={idx} src={s.img} alt={s.title} eager={idx === 0} draggable={false} wrapperClassName="aspect-[4/4.2] w-full shrink-0" className="object-cover" />
          ))}
        </div>
        ...
        <div className="absolute inset-x-0 bottom-0 px-4 pb-3 text-white">
          <div className="grid grid-cols-2 items-center gap-3">
            ...
            <div className="flex items-center justify-end gap-2">
              {slide.picks.map((p) => (
                <Link key={p.id} to="/product/$id" params={{ id: p.id }} className="block w-[96px] shrink-0 text-white">
                  <SmartImage src={p.image} alt={p.name} rounded="rounded-lg" wrapperClassName="aspect-square w-full" className="object-cover" />
                  <p className="mt-1 text-[11px] font-bold drop-shadow">{formatKz(p.price)}</p>
                </Link>
              ))}
            </div>
          </div>
          <div className="mt-2.5 flex justify-center gap-1">
            {slides.map((_, idx) => (
              <button key={idx} onClick={() => goTo(idx)} className={`h-1.5 rounded-full transition-all ${idx === i ? "w-5 bg-white" : "w-1.5 bg-white/50"}`} aria-label={`Slide ${idx + 1}`} />
            ))}
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-12 grid-rows-1 gap-3 h-[340px] lg:h-[380px]">
          ...
          <div className="col-span-6 h-full min-h-0">
            <div className="relative h-full w-full overflow-hidden rounded-xl bg-muted">
              ...
              <button onClick={() => goTo((i - 1 + slides.length) % slides.length)} aria-label="Anterior"
              ...
              <button onClick={() => goTo((i + 1) % slides.length)} aria-label="Próximo"
              ...
              <div className="absolute inset-x-0 bottom-0 p-5 text-white">
                <p className="text-[10px] uppercase tracking-[0.3em] opacity-80">{slide.subtitle}</p>
                <p className="mt-1.5 font-display text-2xl lg:text-3xl font-black leading-tight drop-shadow line-clamp-1">{slide.title}</p>
                <p className="mt-1 max-w-md text-xs opacity-90 line-clamp-1">{slide.caption}</p>
                <div className="mt-2.5 flex items-center gap-3">
                  <button className="rounded-full bg-white px-4 py-1.5 text-[11px] font-bold text-foreground transition hover:bg-white/90">{slide.cta}</button>
                  <div className="flex gap-1.5">
                    {slides.map((_, idx) => (
                      <button key={idx} onClick={() => goTo(idx)} className={`h-1.5 rounded-full transition-all ${idx === i ? "w-6 bg-white" : "w-1.5 bg-white/50"}`} aria-label={`Slide ${idx + 1}`} />
                    ))}
                  </div>
                </div>
              </div>
          ...
          <div className="col-span-3 flex min-h-0 flex-col gap-3">
            {cfg.heroRightTiles.map((it, idx) => (
              ...
              >
                ...
                {it.badge && (
                  <span className="absolute bottom-2 right-3 rounded-full bg-black/60 px-2 py-0.5 text-[9px] font-semibold text-white">{it.badge}</span>
                )}
              </Link>
            ))}
      ...
      {cfg.showQuickStrip && (
        <div className="no-scrollbar mt-3 flex gap-2 overflow-x-auto px-3 md:px-0">
          <div className="relative shrink-0 overflow-hidden rounded-xl" style={{ width: 110, height: 140, background: "var(--gradient-sale)" }}>
            <div className="flex h-full flex-col items-center justify-center px-2 text-center text-white">
              <span className="text-[10px] uppercase tracking-wide opacity-80">A partir de</span>
              <span className="font-display text-3xl font-black drop-shadow">{cfg.quickStripSaverPrice}</span>
              <span className="mt-1 text-[10px] font-semibold">{cfg.quickStripSaverLabel}</span>
            </div>
          </div>
          {cfg.quickStripItems.map((item, idx) => (
            <div key={item.id} className="relative shrink-0 overflow-hidden rounded-xl bg-muted" style={{ width: 110, height: 140 }}>
              <SmartImage
                ...
                className="object-cover"
              />
              <div className="absolute inset-x-0 bottom-0 bg-black/70 py-1 text-center text-[11px] font-medium text-white">{item.label}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {cfg.showCategories && (
        <div className="mt-4 px-3 md:px-0">
          <div className="rounded-2xl bg-card p-3 shadow-[var(--shadow-card)]">
            {cfg.categoriesTitle && (
              <h3 className="mb-2 px-1 font-display text-sm font-black">{cfg.categoriesTitle}</h3>
            )}
            <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-12 gap-3">
              {catStatus === "loading" && !tabCats.length &&
                Array.from({ length: 8 }).map((_, k) => (
                  <div key={`cs-${k}`} className="flex flex-col items-center gap-1.5">
              ...
              {tabCats.map((c) => (
                <Link key={c.slug} to="/category/$slug" params={{ slug: c.slug }} className="flex flex-col items-center gap-1.5">
                  <div className="relative h-16 w-16 md:h-20 md:w-20 overflow-hidden rounded-full bg-gradient-to-br from-muted to-accent">
                    {c.image ? (
                      <SmartImage src={c.image} alt={c.name} rounded="rounded-full" wrapperClassName="h-full w-full" className="object-cover" />
                    ) : (
                      <span className="absolute inset-0 flex items-center justify-center text-2xl">{c.emoji}</span>
                    )}
                  </div>
                  <span className="text-center text-[10px] md:text-xs leading-tight">{c.name}</span>
                </Link>
              ))}
            </div>
          </div>
      ...
      <section className="mt-5 px-3 md:px-0">
        <h2 className="mb-3 font-display text-xl md:text-2xl font-bold">
          {!active.slugs.length ? "Recomendados para você" : `${active.label} — seleção`}
        </h2>
        {prodStatus === "loading" && !filtered.length ? (
          ...
          <div className="columns-2 md:columns-4 lg:columns-5 gap-3 md:gap-4 [column-fill:_balance]">
            {filtered.map((p, i) => (
              <div key={p.id} className="mb-3 md:mb-4 break-inside-avoid">
                <ProductCard product={p} aspect={i % 3 === 1 ? "aspect-[3/4.6]" : i % 3 === 2 ? "aspect-[3/3.4]" : "aspect-[3/4]"} />
              </div>
            ))}
          </div>
          ...
          <p className="py-10 text-center text-sm text-muted-foreground">
            Nenhum produto publicado em <span className="font-semibold">{active.label}</span> por enquanto.
          </p>
        )}
      </section>
    </Layout>
    ...
    <div>
      <div className="flex items-center justify-between rounded-t-2xl bg-gradient-to-r from-orange-50 to-pink-50 px-3 py-2">
        <div className="flex items-center gap-1.5">
          <span className="font-display text-lg font-black text-sale">Super</span>
      ...
      <div className="no-scrollbar -mt-px flex gap-2 overflow-x-auto rounded-b-2xl bg-gradient-to-b from-pink-50 to-background p-3">
        {items.slice(0, 6).map((p) => (
          <Link key={p.id} to="/product/$id" params={{ id: p.id }} className="shrink-0" style={{ width: 120 }}>
            <div className="relative overflow-hidden rounded-lg bg-muted aspect-square">
              <SmartImage src={p.image} alt={p.name} wrapperClassName="h-full w-full" className="object-cover" />
            </div>
            <p className="mt-1 text-xs font-bold text-sale">{formatKz(p.price)}</p>
            {p.oldPrice && (
        ...
        {viral.map((p) => (
          <Link key={p.id} to="/product/$id" params={{ id: p.id }} className="shrink-0" style={{ width: 120 }}>
            <div className="relative overflow-hidden rounded-lg bg-muted aspect-square">
              ...
              <span className="absolute left-1 top-1 rounded-full bg-purple-600 px-1.5 py-0.5 text-[9px] font-black text-white">VIRAL</span>
            </div>
            <p className="mt-1 text-xs font-bold text-purple-700">{formatKz(p.price)}</p>
          </Link>
        ))}
      </div>
    </div>
...
function ErrorState({ onRetry, className = "" }: { onRetry: () => void; className?: string }) {
  return (
    <div className={`flex flex-col items-center justify-center gap-2 rounded-xl bg-muted/50 text-center ${className}`}>
      <p className="text-xs text-muted-foreground">Não foi possível carregar os dados.</p>
      <button onClick={onRetry} className="rounded-full bg-foreground px-4 py-1.5 text-[11px] font-bold text-background">
        Tentar novamente
      </button>
    </div>
  );

  ...
  head: () => ({
    meta: [
      { title: "Meus cupões — Bazarixy" },
      {
        name: "description",
        content:
          "Veja os cupões de desconto disponíveis, os já usados e os expirados, e aplique-os ao finalizar a compra na Bazarixy.",
      },
...
function CouponCard({ c, dim }: { c: Coupon; dim?: boolean }) {
  ...
  return (
    ...
    >
      <span className="absolute left-0 top-0 z-10 bg-brand px-2 py-0.5 text-[10px] font-black uppercase tracking-wide text-brand-foreground">
        Novo
      </span>
      {/* notches */}
      ...
      <div className="flex shrink-0 flex-col items-center justify-center border-b border-dashed border-sale/40 px-3 pb-4 pt-7 sm:w-36 sm:border-b-0 sm:border-r sm:py-7">
        <p className="font-display text-3xl font-black leading-none text-sale sm:text-4xl">
          ...
          {c.type === "percent" && (
            <span className="align-super text-base">%OFF</span>
          )}
        </p>
      ...
      <div className="min-w-0 flex-1 px-4 py-4">
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
          <div className="min-w-0">
            <h3 className="font-display text-base font-black leading-tight sm:text-lg">
              Cupão para todo o site
            </h3>
            <p className="mt-1 text-sm">
              <span className="text-muted-foreground">Código </span>
              <span className="break-all font-mono font-black text-sale">
                {c.code}
              </span>
            </p>
            <p className="mt-0.5 line-clamp-2 text-xs text-muted-foreground">
              {c.description || "Para produtos seleccionados"}
            </p>
          </div>
          <button
            onClick={() => {
        ...
        {open && (
          <ul className="mt-2 space-y-1 text-[11px] text-muted-foreground">
            ...
            <li>
              ·{" "}
              {c.type === "percent"
                ? `Desconto de ${c.value}% sobre o subtotal.`
                : `Desconto de ${formatKz(c.value)} sobre o subtotal.`}
            </li>
            <li>
              ·{" "}
              {c.minOrder > 0
                ? `Válido em pedidos a partir de ${formatKz(c.minOrder)}.`
              ...
              {TABS.map((t) => (
                <button
                  ...
                  className={`relative pb-1 text-sm font-bold ${
                    tab === t
                      ? "text-foreground after:absolute after:inset-x-0 after:-bottom-0.5 after:h-0.5 after:bg-foreground"
            ...
            {list.length === 0 && (
              <div className="border border-dashed border-border py-16 text-center">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  {tab === "Usados"
                    ? "Os cupões que usar aparecem nesta lista."

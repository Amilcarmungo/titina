...
function ShopsPage() {
  ...
  const save = () => {
    ...
    if (modal.id) {
      shopActions.update(modal.id, {
        name: modal.name,
        slug,
        logo: modal.logo || undefined,
        cover: modal.cover || undefined,
  ...
  const readFile = async (key: "logo" | "cover", f: File) => {
    const url = await uploadImageFile(
      f,
      storagePaths.shop(modal?.id ?? "novas", f.name),
    );
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {shops.length} loja(s) · Cada loja tem seus produtos, pedidos e
            avaliações
          </p>
        </div>
        <button
            ...
            <div className="relative px-4 pb-4">
              ...
              <div className="mt-3">
                ...
                <p className="text-[11px] text-muted-foreground">
                  /{s.slug} · {s.ownerName || "Sem responsável"}
                </p>
              ...
              <div className="mt-3 flex items-center justify-between rounded-xl bg-muted/50 px-3 py-2 text-xs">
                <span className="inline-flex items-center gap-1.5">
                  <Package className="h-3.5 w-3.5" /> {counts[s.id] ?? 0}{" "}
                  produto(s)
                </span>
                <Link
                  to="/justina/lojas/$id"
      ...
      >
        {modal && (
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <AdminField label="Responsável">
              <AdminInput
                value={modal.ownerName}
                onChange={(e) =>
                  setModal({ ...modal, ownerName: e.target.value })
                }
            ...
            <AdminField label="Logo">
              <div className="flex items-center gap-2">
                ...
                >
                  <ImageIcon className="h-3.5 w-3.5" /> Enviar
                </label>
                {modal.logo && (
                  <img
                    src={modal.logo}

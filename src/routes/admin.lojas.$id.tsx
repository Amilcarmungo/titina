...
function ShopDetail() {
  ...
  const shopProducts = useMemo(
    () => products.filter((p) => (p.shopId ?? "main") === id),
    [products, id],
  );
  const productIds = new Set(shopProducts.map((p) => p.id));
  const shopOrders = useMemo(
    () =>
      orders.filter((o) => o.items.some((i) => productIds.has(i.productId))),
    [orders, productIds],
  );
  const shopReviews = useMemo(
    () => reviews.filter((r) => productIds.has(r.productId)),
    [reviews, productIds],
  );
      ...
      <div className="overflow-hidden rounded-2xl bg-background shadow-[var(--shadow-card)]">
        <div className="relative aspect-[16/5] bg-gradient-to-br from-muted to-muted/50">
          {shop.cover ? (
            ...
            <div className="grid h-full place-items-center text-6xl opacity-40">
              <StoreIcon />
            </div>
          )}
        </div>
          ...
          <div className="-mt-10 flex flex-wrap items-end gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-muted-foreground">
                /{shop.slug} · Responsável: {shop.ownerName || "—"} · Desde{" "}
                {shop.createdAt}
              </p>

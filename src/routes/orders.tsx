...
const statusMeta: Record<
  OrderStatus,
  { color: string; label: string; icon: typeof Clock; phrase: string }
> = {
  unpaid: {
    color: "text-sale",
    ...
    icon: Clock,
    phrase:
      "O pagamento deste pedido ainda não foi concluído. Finalize-o para garantir os seus artigos.",
  },
  processing: {
    ...
    icon: Package,
    phrase:
      "O seu pedido está a ser tratado pela nossa equipa e pelas lojas envolvidas.",
  },
  shipped: {
    ...
    icon: Truck,
    phrase:
      "O pedido já saiu para entrega. Acompanhe abaixo cada etapa do envio.",
  },
  review: {
  ...
  returns: {
    ...
    icon: RotateCcw,
    phrase:
      "O processo de devolução está em curso. Informamos assim que o reembolso for emitido.",
  },
};
    ...
    <Layout hideHeader>
            ...
            {TABS.map((t) => {
              ...
              return (
                ...
                >
                  ...
                  {isActive && (
                    <span className="absolute -bottom-px left-1/2 h-0.5 w-8 -translate-x-1/2 rounded-full bg-brand-strong" />
                  )}
                </Link>
              );
            })}
      ...
      <div className="mt-3 space-y-3 px-3 md:px-0 pb-8">
        {list.length === 0 ? (
          ...
          list.map((o) => (
            <OrderCard key={o.id} order={o} onReview={() => setReviewFor(o)} />
          ))
        )}
      </div>

      {reviewFor && (
        <ReviewModal order={reviewFor} onClose={() => setReviewFor(null)} />
      )}
    </Layout>
  );
}
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-border/70">
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span className={`text-xs font-bold truncate ${meta.color}`}>
            {order.paymentProof && order.status === "processing"
          ...
          <div className="ml-auto text-right">
            ...
            >
              {order.shipping?.isFree
                ? "Frete grátis"
                : `Frete ${formatKz(order.shipping?.chargedFee ?? 0)}`}
            </p>
          </div>
        </div>
        ...
        {order.status === "review" ? (
          <div className="mt-3 rounded-xl bg-brand/15 px-3 py-2.5">
            <p className="text-[12px] font-bold text-brand-strong">
              Avalie e ganhe {POINTS_PER_REVIEW} pontos
            </p>
            <p className="mt-0.5 text-[11px] leading-relaxed text-muted-foreground">
              Conte como foi a sua experiência. Os pontos entram na sua conta
              assim que publicar a avaliação.
            </p>
          </div>
                  ...
                  return (
                    <button
                      ...
                      title={
                        canConfirm
                          ? "Confirmar receção"
                          : "Disponível quando a entrega for confirmada pela loja"
                      }
                      onClick={() => {
...
function RefundBlock({
  ...
  if (!r) {
    return (
      <div className="mt-3 rounded-xl border border-border/70 px-3 py-2.5">
        ...
        <p className="mt-0.5 text-[11px] leading-relaxed text-muted-foreground">
          Está tudo pronto. Indique onde quer receber o dinheiro e confirme as
          condições — assim que a encomenda chegar ao nosso armazém, emitimos o
          reembolso.
        </p>
        <button
          onClick={onRequest}
  ...
  return (
    <div className="mt-3 rounded-xl bg-brand/15 px-3 py-2.5">
      <p className="animate-pulse text-[11px] font-bold text-brand-strong">
        Reembolso em processamento
      </p>
      <p className="mt-0.5 text-[11px] leading-relaxed text-muted-foreground">
        Vamos transferir {formatKz(order.total)} para <b>{r.method}</b> ·{" "}
        {r.account} ({r.holder}). Pedido em {r.requestedAt}.
      </p>
    </div>
  );
}
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              Pedido #{order.id} · {formatKz(order.total)}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <label className="mt-3 flex items-start gap-2 text-[11px] leading-relaxed">
          ...
          />
          <span>
            Confirmo que o artigo está sem uso, com a embalagem original, e que
            devolvo a encomenda completa. O reembolso é emitido após a
            verificação no armazém.
          </span>
        </label>
...
function visibleStages(
  ...
  const list = STAGE_FLOW.filter((s) => s !== "payment_rejected").filter(
    (s) => {
                ...
                <span
                  className={`mt-0.5 grid h-3.5 w-3.5 shrink-0 place-items-center rounded-full ${
                    rejected
                      ...
                      : isCurrent
                        ? "animate-pulse bg-brand/40 text-brand-strong"
        ...
        <ul className="mt-2 space-y-1 border-t border-border/60 pt-2">
          {[...pkg.timeline].reverse().map((ev, i) => (
            <li key={`${ev.stage}-${i}`} className="flex gap-2 text-[10px]">
              <span
                className={`mt-1 h-1 w-1 shrink-0 rounded-full ${i === 0 ? "bg-brand-strong" : "bg-muted-foreground/40"}`}
              />
              <span>
...
function ReviewModal({
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.error(
        "Não conseguimos validar sua compra. Verifique se o pedido foi entregue corretamente.",
      );
    } finally {
      ...
      >
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              Conte a sua experiência — ajuda outros clientes a decidir.
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="mt-1 flex gap-1">
            {[1, 2, 3, 4, 5].map((n) => (
              <button key={n} onClick={() => setRating(n)}>
                <Star
                  className={`h-8 w-8 transition ${n <= rating ? "fill-gold text-gold" : "text-muted-foreground/30"}`}
                />
              </button>
            ))}
          </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
      ...
      <p className="mt-4 text-sm font-semibold">
        Nenhum pedido {meta.label.toLowerCase()}
      </p>
      <p className="mt-1 text-xs text-muted-foreground">
        Quando tiveres um pedido, aparece aqui.
      </p>
      <Link
        to="/"

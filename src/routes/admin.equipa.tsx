...
import { ROLE_LABEL, usePermission, type StaffMember, type StaffRole } from "@/lib/firebase/roles";

export const Route = createFileRoute("/admin/equipa")({
  head: () => ({
    meta: [
      { title: "Equipa — Admin Bazarixy" },
      { name: "description", content: "Adicione membros da equipa e defina o sector de cada um." },
...
function EquipaPage() {
  ...
  useEffect(() => {
    const db = getDb();
    if (!db || !canManage) { setLoading(false); return; }
    return onSnapshot(
      collection(db, "staff"),
      (snap) => {
        setMembers(snap.docs.map((d) => ({ uid: d.id, ...(d.data() as Omit<StaffMember, "uid">) })));
        setLoading(false);
      },
      () => setLoading(false),
    );
  ...
  const add = async () => {
    ...
    if (!db) return;
    const id = uid.trim();
    if (id.length < 8) { toast.error("Indique o UID do utilizador (Firebase Auth)."); return; }
    setBusy(true);
    try {
      await setDoc(
        doc(db, "staff", id),
  ...
  if (!canManage) {
    return (
      <div className="rounded-2xl border border-dashed border-border p-10 text-center">
        <ShieldCheck className="mx-auto h-10 w-10 text-muted-foreground" strokeWidth={1.3} />
        <p className="mt-3 font-display text-lg font-black">Sem acesso</p>
        <p className="mt-1 text-sm text-muted-foreground">Só um administrador pode gerir a equipa.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Adicione membros e escolha o sector de cada um. As permissões são validadas no servidor.</p>
      </div>

      <div className="rounded-2xl border border-border bg-card p-4">
        <p className="flex items-center gap-2 text-sm font-bold"><UserPlus className="h-4 w-4" /> Adicionar membro</p>
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          ...
          <label className="text-xs font-semibold">
            ...
            <span className="mt-1 block text-[11px] font-normal text-muted-foreground">{ROLE_HINT[role]}</span>
          </label>
          <label className="text-xs font-semibold">
            Loja (opcional)
            <input
        ...
        >
          {busy && <Loader2 className="h-4 w-4 animate-spin" />} Adicionar à equipa
        </button>
      </div>

      <div className="rounded-2xl border border-border bg-card">
        <p className="border-b border-border px-4 py-3 text-sm font-bold">Membros ({members.length})</p>
        {loading ? (
          <div className="space-y-2 p-4">
            {[0, 1, 2].map((i) => <div key={i} className="h-14 animate-pulse rounded-xl bg-muted" />)}
          </div>
        ) : members.length === 0 ? (
          <p className="p-6 text-center text-sm text-muted-foreground">Ainda não há membros.</p>
        ) : (
          <ul className="divide-y divide-border">
            {members.map((m) => (
              <li key={m.uid} className="flex flex-wrap items-center gap-3 px-4 py-3">
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-bold">{m.name || m.uid}</p>
                  <p className="truncate text-[11px] text-muted-foreground">
                    {ROLE_LABEL[m.role] ?? m.role}{m.shopId ? ` · ${m.shopId}` : ""} · {m.uid}
                  </p>
                </div>
                <button

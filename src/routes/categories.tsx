...
import { ChevronLeft, Camera, Search, Trophy, X, Clock, Flame, SlidersHorizontal } from "lucide-react";
...
export const Route = createFileRoute("/categories")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Pesquise produtos, explore categorias e descubra as tendências mais vendidas na Bazarixy." },
      { property: "og:title", content: "Buscar produtos — Bazarixy" },
      { property: "og:description", content: "Pesquise produtos e explore todas as categorias da Bazarixy." },
...
const discovery = [
  "acessórios", "vestidos de verão", "vestidos para mulheres", "biquíni",
  "trajes de banho", "sapatos", "roupa esportiva feminina", "moletons",
];
...
function SearchPage() {
  ...
  const hotList = useMemo(() => [...all].sort((a, b) => (b.sold ?? 0) - (a.sold ?? 0)).slice(0, 12), [all]);
      ...
      <div className="mx-auto md:max-w-6xl">
        ...
        <div className="sticky top-0 z-30 border-b border-border/70 bg-background/95 backdrop-blur">
          <div className="flex items-center gap-2 px-2 py-2 md:px-0 md:py-4">
            <button onClick={() => router.history.back()} className="shrink-0 p-2 md:hidden" aria-label="Voltar">
              <ChevronLeft className="h-5 w-5" />
            </button>
            <form
              onSubmit={(e) => { e.preventDefault(); remember(q); }}
            ...
            >
              ...
              />
              {q && (
                <button type="button" onClick={() => setQ("")} className="shrink-0 p-1.5 text-muted-foreground" aria-label="Limpar">
                  <X className="h-4 w-4" />
                </button>
              )}
              <button type="button" className="shrink-0 p-1.5 text-muted-foreground" aria-label="Buscar por foto"><Camera className="h-5 w-5" /></button>
              <button type="submit" className="grid h-9 w-12 shrink-0 place-items-center rounded-none bg-brand-strong text-white" aria-label="Buscar">
          ...
          <div className="no-scrollbar overflow-x-auto pb-2">
            <div className="flex gap-2 px-3 md:px-0">
              ...
              {cats.map((c) => (
                <Chip key={c.slug} active={cat === c.slug} onClick={() => setCat(cat === c.slug ? null : c.slug)}>
                  {c.name}
                </Chip>
              ))}
            </div>
        ...
        {searching ? (
          <div className="px-3 py-3 md:px-0">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
              <p className="text-xs text-muted-foreground">
                <b className="text-foreground">{results.length}</b> resultado(s){q ? ` para “${q}”` : ""}
              </p>
              ...
              {results.map((p) => (
                <Link key={p.id} to="/product/$id" params={{ id: p.id }}
                  ...
                  <div className="aspect-square overflow-hidden bg-muted">
                    <SmartImage src={p.image} alt={p.name} wrapperClassName="absolute inset-0 h-full w-full" className="object-cover transition duration-500 group-hover:scale-105" />
                  </div>
                  <div className="p-2.5">
                    <p className="line-clamp-2 text-xs leading-tight">{p.name}</p>
                    <p className="mt-1 text-sm font-black text-sale">{formatKz(p.price)}</p>
                  </div>
                </Link>
              ))}
            </div>
            ...
            {results.length === 0 && (
              <div className="py-16 text-center">
                ...
                <p className="mt-1 text-sm text-muted-foreground">Tente outra palavra ou explore as categorias abaixo.</p>
                <div className="mt-4 flex flex-wrap justify-center gap-2">
                  {discovery.slice(0, 5).map((t) => (
                    <button key={t} onClick={() => { setQ(t); setCat(null); }} className="rounded-none border border-border px-3 py-1.5 text-xs font-semibold hover:bg-muted">{t}</button>
                  ))}
                </div>
              </div>
            )}
          ...
          <div className="grid gap-6 px-4 py-4 md:grid-cols-[1fr_320px] md:px-0">
            <div className="space-y-8">
              {recent.length > 0 && (
                <section>
                  <div className="flex items-center justify-between">
                    <h2 className="flex items-center gap-1.5 text-lg font-black"><Clock className="h-4 w-4 text-muted-foreground" /> Buscas recentes</h2>
                    <button onClick={() => { setRecent([]); localStorage.removeItem(RECENT_KEY); }} className="text-xs font-bold text-muted-foreground hover:text-foreground">Limpar</button>
                  </div>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {recent.map((t) => (
                      <button key={t} onClick={() => setQ(t)} className="rounded-none bg-muted px-3.5 py-2 text-sm hover:bg-muted/70">{t}</button>
                    ))}
                  </div>
                </section>
              )}
              ...
              <section>
                <h2 className="flex items-center gap-1.5 text-lg font-black"><Flame className="h-4 w-4 text-sale" /> Descobrir buscas</h2>
                <div className="mt-3 flex flex-wrap gap-2">
                  {discovery.map((t) => (
                    <button key={t} onClick={() => { setQ(t); remember(t); }}
                  ...
                  {hotList.map((p, i) => (
                    <Link key={p.id} to="/product/$id" params={{ id: p.id }} className="group">
                      <div className="relative aspect-square overflow-hidden rounded-none bg-muted">
                        <SmartImage src={p.image} alt={p.name} wrapperClassName="absolute inset-0 h-full w-full" className="object-cover transition duration-500 group-hover:scale-105" />
                        <span className="absolute bottom-1 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-none px-2 py-0.5 text-[10px] font-black text-white" style={{ background: "#111111" }}>
                          TOP {i + 1}
                        </span>
                      </div>
                      <p className="mt-1.5 line-clamp-2 text-[11px] leading-tight">{p.name}</p>
                    </Link>
                  ))}
                </div>
              </section>
                ...
                {cats.map((c) => (
                  <Link key={c.slug} to="/category/$slug" params={{ slug: c.slug }}
                    ...
                    <div className="flex aspect-square items-center justify-center bg-muted">
                      {c.image ? (
                        <SmartImage src={c.image} alt={c.name} wrapperClassName="absolute inset-0 h-full w-full" className="object-cover transition duration-500 group-hover:scale-105" />
                      ) : (
                        <span className="text-4xl">{c.emoji}</span>
                      )}
                    </div>
                    <div className="p-2 text-center text-xs font-semibold">{c.name}</div>
                  </Link>
                ))}
              </div>
            </aside>

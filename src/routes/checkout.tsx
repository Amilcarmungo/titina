  ...
  head: () => ({
    meta: [
      { title: "Finalizar Compra — Bazarixy" },
      {
        name: "description",
        content:
          "Complete seu pedido em 3 etapas: endereço, confirmação e pagamento.",
      },
    ],
  }),
        ...
        {STEPS.map((s, i) => {
          ...
          return (
            <div key={s.id} className="flex flex-1 items-center last:flex-none">
              ...
              {i < STEPS.length - 1 && (
                <div
                  className={`mx-2 h-[3px] flex-1 rounded-full ${step > s.id ? "bg-sale" : "bg-muted"}`}
                />
              )}
            </div>
          );
...
function CheckoutPage() {
  ...
  const subtotal = items.reduce(
    (s, i) => s + (i.unitPrice ?? i.product.price) * i.qty,
    0,
  );
      ...
      <Layout title="Pedido confirmado" showBack hideBottomNav>
        <div className="mx-auto max-w-lg px-6 py-16 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Obrigado pela sua compra. Enviaremos atualizações por e-mail.
          </p>
            ...
            <>
              ...
              <section className="rounded-xl bg-card p-4 shadow-sm">
                ...
                <Field
                  ...
                  error={
                    !address.name ? "Por favor, insira seu nome" : undefined
                  }
                />
              ...
              <section className="flex items-center justify-between rounded-xl bg-card p-4 shadow-sm">
                ...
                >
                  <span
                    className={`absolute top-0.5 h-5 w-5 rounded-full bg-white shadow transition ${address.isDefault ? "left-[22px]" : "left-0.5"}`}
                  />
                </button>
              </section>
            </>
            ...
            <>
              ...
              {items.map((item) => (
                ...
                >
                  <div className="flex items-center justify-between text-xs">
                    <span className="rounded bg-yellow-200 px-1.5 py-0.5 font-bold text-yellow-900">
                      Choice
                    </span>
                  ...
                  <div className="mt-3 flex gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="mt-1 font-bold">
                        {formatKz(item.unitPrice ?? item.product.price)}
                      </p>
                      <div className="mt-2 inline-flex items-center rounded-md border border-border">
                        <button
                          onClick={() =>
                            actions.updateQty(item.idx, item.qty - 1)
                          }
                  ...
                  <div className="mt-3 border-t border-border pt-2 text-sm">
                    <p className="font-bold">
                      ...
                      {shipping?.isFree
                        ? "Frete grátis"
                        : formatKz(shipping?.chargedFee ?? 0)}
                    </p>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[11px] text-muted-foreground">
                      {shippingSettings.freeShippingEnabled
                        ? `Frete grátis em compras a partir de ${formatKz(shippingSettings.freeShippingThreshold)}.`
                        : "Escolha a forma de entrega para ver o custo."}
                    </p>
                  </div>
                ...
                <div className="mt-3 space-y-2">
                  {shippingOptionsForOrder.map((option) => (
                    ...
                    >
                      ...
                      >
                        {option.isFree ? "Grátis" : formatKz(option.chargedFee)}
                      </span>
                    </button>
                  ))}
                  {shippingOptionsForOrder.length === 0 && (
                    <p className="text-xs text-muted-foreground">
                      Não há opções de entrega ativas. Contacte o suporte.
                    </p>
                  )}
                </div>
              </section>
              ...
              <section className="rounded-xl bg-card p-4 shadow-sm text-sm">
                ...
                <div className="mt-2 flex justify-between">
                  ...
                  <span
                    className={
                      shipping?.isFree ? "font-bold text-emerald-700" : ""
                    }
                  >
                    {shipping?.isFree
                      ? "Grátis"
                ...
                <div className="mt-2 flex justify-between text-base font-bold">
                  <span>Total</span>
                  <span className="text-sale">{formatKz(total)}</span>
                </div>
                <p className="mt-4 text-[11px] text-muted-foreground">
                  Ao clicar "Fazer o pedido", eu afirmo que li e estou de acordo{" "}
                  <Link to="/termos" className="text-sky-600">
            ...
            <>
              ...
              <section className="rounded-xl bg-card p-4 text-xs shadow-sm">
                ...
                <p className="mt-1 text-muted-foreground">
                  Ao tocar em «Pagar» vai para a página do método escolhido, com
                  as instruções e o envio do comprovativo.
                </p>
              </section>

              <div className="rounded-xl bg-card p-4 text-xs shadow-sm">
                <p className="flex items-center gap-2 font-bold">
                  <ShieldCheck className="h-4 w-4 text-emerald-600" /> Bazarixy
                  protege suas informações do pagamento
                </p>
    ...
    <div className="mt-3">
      <div className="rounded-lg border border-border px-3 py-2">
        ...
        <input
          value={value}
          inputMode={
            format === "phone" || format === "numeric" ? "numeric" : inputMode
          }

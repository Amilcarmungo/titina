...
type Row = { uid: string; email?: string; name?: string | null; photoURL?: string | null; points?: number; createdAt?: unknown };
...
function UsersPage() {
  ...
  useEffect(() => {
    const db = getDb();
    if (!db || !staff) { setState("error"); return; }
    return onSnapshot(
      collection(db, "users"),
      (snap) => {
    ...
    <div className="space-y-4">
      <header className="flex flex-wrap items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground">{rows.length} conta(s) registada(s) na Bazarixy.</p>
        </div>
      ...
      {state === "ready" && (
        list.length === 0 ? (
          <p className="rounded-xl border border-dashed border-border p-10 text-center text-sm text-muted-foreground">Nenhum utilizador encontrado.</p>
        ) : (
          <div className="overflow-hidden rounded-2xl border border-border bg-card">
            <div className="hidden grid-cols-[1fr_auto_auto] gap-4 border-b border-border px-4 py-2.5 text-[11px] font-black uppercase tracking-wider text-muted-foreground sm:grid">
              <span>Conta</span><span>Pontos</span><span>ID</span>
            </div>
            <ul className="divide-y divide-border">
              {list.map((r) => (
                <li key={r.uid} className="grid gap-2 px-4 py-3 sm:grid-cols-[1fr_auto_auto] sm:items-center sm:gap-4">
                  <div className="flex min-w-0 items-center gap-3">
                    <span className="grid h-9 w-9 shrink-0 place-items-center overflow-hidden rounded-full bg-muted text-xs font-black">
                      {r.photoURL ? <img src={r.photoURL} alt="" className="h-full w-full object-cover" /> : (r.email?.[0] ?? "?").toUpperCase()}
                    </span>
                    <div className="min-w-0">
                      <p className="truncate text-sm font-bold">{r.name || r.email || "Sem nome"}</p>
                  ...
                  <code className="truncate text-[11px] text-muted-foreground">{r.uid}</code>
                </li>
              ))}
            </ul>
          </div>

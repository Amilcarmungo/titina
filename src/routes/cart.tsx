  ...
  head: () => ({
    meta: [
      { title: "Sacola — Bazarixy" },
      {
        name: "description",
        content:
          "Reveja os produtos da sua sacola, escolha o que quer pagar agora e finalize a compra na Bazarixy.",
      },
...
function Recommendations({
  ...
  const list = [...sameCat, ...pool.filter((p) => !sameCat.includes(p))].slice(
    0,
    8,
  );
...
function CartPage() {
  ...
  const ungrouped = items.filter(
    (i) => !shops.some((s) => s.id === (i.product.shopId ?? "main")),
  );
  if (ungrouped.length)
    groups.push({
      shop: {
        ...
        <div className="mx-auto max-w-3xl pb-32">
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            <span className="text-sm font-bold">
              Seleccionar tudo ({items.length})
            </span>
            {selected.length > 0 && (
              <button
                onClick={() => {
                  if (confirm(`Remover ${selected.length} item(ns)?`))
                        ...
                        <div className="flex min-w-0 flex-1 flex-col">
                          ...
                          <div className="mt-auto flex items-center justify-between gap-2">
                            <div className="min-w-0">
                              <span className="block font-black text-sale">
                                {formatKz(price)}
                              </span>
                              {item.product.oldPrice && (
                                <span className="block text-[11px] text-muted-foreground line-through">
                                  {formatKz(item.product.oldPrice)}
                                </span>
                              )}
                            </div>
          ...
          <div className="mx-3 mt-2 space-y-2 rounded-2xl bg-card p-4 text-sm shadow-[var(--shadow-card)]">
            <div className="flex justify-between">
              <span className="text-muted-foreground">Itens seleccionados</span>
            ...
            <p className="text-[11px] text-muted-foreground">
              O valor final é confirmado depois de escolher a entrega e informar
              o endereço.
            </p>
            <div className="flex justify-between border-t border-border pt-2 text-base font-black">
              <span>Total estimado</span>
              <span className="text-sale">{formatKz(estimatedTotal)}</span>
            </div>
          </div>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-30 border-t border-border bg-background px-3 py-2.5">
            <div className="mx-auto flex max-w-3xl items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <div className="truncate font-black text-sale">
                  {formatKz(estimatedTotal)}
                </div>
              </div>
              {selected.length === 0 ? (
                <span className="ml-auto rounded-full bg-muted px-8 py-3 text-center text-sm font-bold text-muted-foreground">
                  Continuar (0)
                </span>
              ) : (
                <Link

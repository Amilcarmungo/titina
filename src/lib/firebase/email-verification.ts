...
export async function requestSignupVerification(email: string, name?: string) {
...
export async function verifySignupVerification(code: string) {
  const response = await fetch("/api/signup-verification", {
    method: "POST",
    headers: { "content-type": "application/json" },
...
export async function confirmVerificationCode(
  ...
  await setDoc(
    doc(db, "users", uid),
    { uid, emailVerified: true, verifiedAt: serverTimestamp() },
    { merge: true },
  );
  return { ok: true };
}

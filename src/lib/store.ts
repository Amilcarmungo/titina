...
type CartItem = {
  ...
  variantLabel?: string;
  unitPrice?: number;
  /** Foto da variante escolhida (mostrada no carrinho e nos pedidos). */ image?: string;
};
...
export const actions = {
    ...
    if (existing) {
      state = {
        ...
        cart: state.cart.map((c) =>
          c === existing ? { ...c, qty: c.qty + item.qty, selected: true } : c,
        ),
      };
    } else {
  ...
  toggleSelected(idx: number) {
    state = {
      ...
      cart: state.cart.map((c, i) =>
        i === idx ? { ...c, selected: c.selected === false } : c,
      ),
    };
    emit();
  },
  setAllSelected(value: boolean) {
    state = {
      ...state,
      cart: state.cart.map((c) => ({ ...c, selected: value })),
    };
    emit();
  },
  updateQty(idx: number, qty: number) {
    ...
    state = {
      ...state,
      cart: state.cart.map((c, i) => (i === idx ? { ...c, qty } : c)),
    };
    emit();
  },
  ...
  openLogin() {
    state = { ...state, loginOpen: true };
    emit();
  },
  closeLogin() {
    state = { ...state, loginOpen: false };
    emit();
  },
  openCartDrawer() {
    state = { ...state, cartDrawerOpen: true };
    emit();
  },
  closeCartDrawer() {
    state = { ...state, cartDrawerOpen: false };
    emit();
  },
  openFavDrawer() {
    state = { ...state, favDrawerOpen: true };
    emit();
  },
  closeFavDrawer() {
    state = { ...state, favDrawerOpen: false };
    emit();
  },
  openProfileDrawer() {
    state = { ...state, profileDrawerOpen: true };
    emit();
  },
  closeProfileDrawer() {
    state = { ...state, profileDrawerOpen: false };
    emit();
  },
  openMenu() {
  ...
  setMenu(v: boolean) {
    state = { ...state, menuOpen: v };
    emit();
  },
  openNotifDrawer() {
    state = { ...state, notifDrawerOpen: true };
    emit();
  },
  closeNotifDrawer() {
    state = { ...state, notifDrawerOpen: false };
    emit();
  },
};

...
export async function sendWithResend(input: {
  ...
  const from = process.env["EMAIL_FROM"] ?? "Bazarixy <nao-responder@bazarixy.com>";
 ...
 */
export async function isActiveStaff(idToken: string, uid: string): Promise<boolean> {
  ...
  const doc = (await res.json()) as { fields?: { active?: { booleanValue?: boolean }; role?: { stringValue?: string } } };
  const role = doc.fields?.role?.stringValue;
  return doc.fields?.active?.booleanValue === true

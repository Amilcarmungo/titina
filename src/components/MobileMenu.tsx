...
import { ChevronRight, DollarSign, Globe, X, ChevronDown, Home, Heart, Headset, User } from "lucide-react";
...
export function MobileMenu({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  const topTabs = cfg.homeTabs.length ? cfg.homeTabs : [{ id: "t-all", label: "Tudo", slugs: [] as string[] }];
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="left" className="w-[85vw] max-w-sm p-0 flex flex-col gap-0 pb-16">
        ...
        <div className="flex items-center justify-between border-b border-border pl-2 pr-1">
            ...
            {topTabs.map((t, i) => (
              <button
                ...
                className={`relative whitespace-nowrap text-sm font-bold ${
                  i === tab ? "text-foreground after:absolute after:-bottom-2.5 after:left-0 after:right-0 after:h-0.5 after:bg-foreground" : "text-muted-foreground"
                }`}
              >
                {t.label}
              </button>
          ...
          <button onClick={() => onOpenChange(false)} className="grid h-9 w-9 place-items-center rounded bg-foreground text-background">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <ul className="py-1">
            ...
            {visibleCats.length === 0 && (
              <li className="px-4 py-6 text-center text-sm text-muted-foreground">Sem categorias nesta aba.</li>
            )}
          </ul>
          ...
          >
            BAZARIXY Info
            <ChevronDown className={`h-4 w-4 transition-transform ${infoOpen ? "" : "-rotate-90"}`} />
          </button>
          {infoOpen && (
            <ul className="pb-4">
              {infoLinks.map((l) => (
                <li key={l.label} className="text-[15px]">
                  {l.to ? (
                    <Link to={l.to} onClick={() => onOpenChange(false)} className="block px-4 py-2.5">{l.label}</Link>
                  ) : l.href ? (
                    <a href={l.href} target="_blank" rel="noreferrer" className="block px-4 py-2.5">{l.label}</a>
                  ) : (
        ...
        <nav className="absolute inset-x-0 bottom-0 grid grid-cols-4 border-t border-border bg-background">
          {[
            ...
            const Icon = it.icon;
            return (
              <Link key={it.to} to={it.to} onClick={() => onOpenChange(false)}

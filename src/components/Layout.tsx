...
export function Layout({
  ...
  const navTabs = cfg.homeTabs.length
    ? cfg.homeTabs
  ...
  return (
    ...
    >
      ...
      {!hideHeader && !simpleHeader && (
        <header
          className={`${headerPositionCls} top-0 left-0 right-0 z-40 md:hidden`}
        >
          <div className="relative">
            ...
            <div className="relative grid h-14 grid-cols-[1fr_auto_1fr] items-center gap-2 px-3 text-white">
              ...
              <div className="justify-self-center">
                {title ? (
                  <h1 className="truncate text-base font-semibold drop-shadow">
                    {title}
                  </h1>
                ) : (
                  <Link to="/" className="flex items-center">
      ...
      <main
        className={
          hideTopNav && hideHeader
            ? "mx-auto w-full max-w-[980px] px-0 py-0 md:px-0 md:py-0"
            : "mx-auto md:max-w-7xl md:px-6 md:py-6"
        }
      >
        {children}
      </main>
                ...
                >
                  <span className="relative">
                    ...
                    {it.to === "/cart" && cartCount > 0 && (
                      <span className="absolute -right-2 -top-1.5 grid h-4 min-w-4 place-items-center rounded-full bg-brand px-1 text-[9px] font-bold text-brand-foreground">
                        {cartCount}
                      </span>
                    )}
                  </span>
...
function BrandLogo({ className = "" }: { className?: string }) {
  return (
    <span className={`relative inline-flex items-center ${className}`}>
      ...
      />
      <span className="hidden font-display text-xl font-black tracking-tight text-brand-strong">
        Bazarixy
      </span>
    </span>
  );
